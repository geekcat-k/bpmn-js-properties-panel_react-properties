...
function ElementProperties(props) {
  ...
  function updateInputData(inputData) {
    ...
    <div className="element-properties" key={element.id}>
      ...
      {is(element, "bpmn:Task") && (
        <fieldset>
          <label>Input</label>
          <input
            value={element.businessObject.get("custom:inputData") || ""}
            onChange={(event) => {
      ...
      {is(element, "bpmn:Task") && (
        <fieldset>
          <label>ETL定義</label>
          <span>{element.businessObject.name || ""}</span>
          <button onClick={openEtlWindow}>設定</button>
        </fieldset>
      )}
